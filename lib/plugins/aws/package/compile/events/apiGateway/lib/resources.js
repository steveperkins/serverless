...
module.exports = {
  ...
  getResourceId(path) {
    ...
    if (!this.apiGatewayResources) {
      throw new Error("No API Gateway resources found")
    }
    if (!this.apiGatewayResources[path]) {
      // The paths in apiGatewayResources switch between leading slash and no slash depending on whether the first resource has a leading slash
